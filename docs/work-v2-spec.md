# ТЗ: Работа v2 + единый Avalone shell

> Цель: довести приложение Работа до production-ready состояния и устранить ощущение «отрыва» от платформы Avalone. Все приложения (Финансы, Работа, будущие ветки) должны восприниматься пользователем как единая платформа с общей навигацией, авторизацией и визуальным языком.

---

## 1. Продуктовая постановка

### 1.1. Что такое Работа
Работа — это ветка Avalone для рабочей координации. Первая функция: организация поездок людей на работу и с работы. Все участники равноправны: любой может отметить себя и любого другого участника поездки ролью «Водитель / Пассажир / Не еду».
...
- Единый Avalone shell (верхняя панель + навигация), который виден и в Работа, и в Финансы, и на портале.
...
Проблема: когда пользователь переходит из портала Avalone в Финансы или Работа, он чувствует, что «вышел» из платформы. Единственная связь — кнопка «Назад в Avalone».
...
Портал Avalone (hero banner уже показывает Работа как запущенную ветку):

![Avalone portal](assets/current-avalone-portal.png)

Текущее приложение Работа (MVP поездок):

![Работа app](assets/current-work-app.png)

Текущее приложение Финансы (отдельное, без общего shell):

![Финансы app](assets/current-counta-app.png)
...
1. **Global top bar** (всегда наверху):
   ...
   - Переключатель веток / приложений (Работа, Финансы, и т.д.).
   - Глобальный поиск.
   - Уведомления.
   - Профиль / выход.
   - Тема / язык.
...
2. **Contextual sidebar or bottom nav** (внутри приложения):
   - Работа: Поездки, Статистика, Уведомления, Настройки.
   - Финансы: Счета, Операции, Аналитика, Настройки.

3. **Breadcrumbs**:
   - Avalone → Работа → Поездка «На работу 26 июня».
...
6. **Cross-app links**:
   - Из Работа можно перейти в Финансы одним кликом.
   - Из Финансы — в Работа.
...
```
┌─────────────────────────────────────────────────────────────┐
│  ◈ Avalone    [Работа ▼]  [🔍 Поиск]  [🔔]  [🌙]  [RU ▼]  [👤] │  ← Global top bar
├──────────┬──────────────────────────────────────────────────┤
│          │  [Avalone] / [Работа] / Поездки                   │  ← Breadcrumbs
│  [🛠️]    │                                                  │
│  Поездки │  ┌──────────────────────────────────────────┐   │
│          │  │  Создать поездку                         │   │
│  [📊]    │  └──────────────────────────────────────────┘   │
...
2. **Branch switcher** (`Работа ▼`) — выпадающее меню со списком активных веток:
   - Работа
   - Финансы (Финансы)
...
`Avalone > Работа > Поездки > На работу 26 июня`
...
- Рекомендуется Вариант B для текущего стека: вынести `avalone-shell.html`, `avalone-shell.css`, `avalone-shell.js` в отдельный репозиторий или директорию и подключать в Финансы/Работа/Работа как git submodule или просто копировать при сборке.
- Все приложения используют один CSS-файл дизайн-системы.

---

## 4. Работа — детальные экраны
...
## 5. Финансы — обновления для единства

### 5.1. Заменить верхнюю панель Финансы на Avalone shell
- Убрать отдельную кнопку «Назад в Avalone».
- Вместо неё — global top bar с branch switcher, где Финансы — текущая ветка.
- Сохранить bottom nav Финансы, но привести стиль к дизайн-системе Avalone.

### 5.2. Breadcrumbs
- Avalone → Финансы → Счета / Операции / Аналитика.

### 5.3. Единый профиль
- Профиль редактируется в Avalone, в Финансы только отображается.

### 5.4. Переходы между приложениями
- Из Финансы можно перейти в Работа через branch switcher.
...
### 7.6. PWA
- Manifest для Работа обновлён: name «Работа — Avalone», иконки, theme-color.
...
1. Пользователь в Работа.
2. Нажимает branch switcher в top bar.
3. Выбирает Финансы.
4. Браузер переходит на `avalone.online/finance`.
...
- [ ] Avalone shell реализован и подключён к порталу, Работа, Финансы.
- [ ] Все приложения используют единую дизайн-систему (CSS-переменные, компоненты, иконки).
- [ ] Работа имеет все экраны из раздела 4.
...
4. Скриншоты всех экранов Работа на desktop и mobile.
...
2. **Фаза 2 — Работа UI/UX** (1–1.5 недели)
   - Все экраны Работа.
   - Мобильная адаптация.
   - PWA.
...
4. **Фаза 4 — Финансы + портал** (3–5 дней)
   - Подключить shell к Финансы.
   - Причесать портал.
