...
> Проблема, которую обнаружил оператор: раньше ветки «Финансы» и «Работа» воспринимались как самостоятельные приложения со своей авторизацией. Это неудобно, раздроблено и противоречит идее единой метавселенной.
> Решение: авторизация должна быть на уровне Avalone. Все ветки используют один аккаунт.

## Проблема

Раньше:
- Ветка «Финансы» — свои пользователи, своя авторизация.
- Ветка «Работа» — свои пользователи, своя авторизация.
- Avalone — портал без авторизации.

Последствия:
- Пользователь регистрируется/входит в каждой ветке отдельно.
- Профиль и репутация размазаны по разным базам.
- Переход из портала в ветку — это выход из одного приложения и вход в другое.
...
- Ветки — это модули внутри Avalone, а не независимые приложения.
...
4. При переходе в ветку (Финансы / Работа) Avalone передаёт токен.
5. Модуль проверяет токен у Avalone и получает профиль пользователя.
...
- `/auth/introspect` — проверка токена модулем.

Модули становятся OIDC Clients:
- Ветки «Финансы» и «Работа» доверяют Avalone.
...
Avalone хранит сессию. Модули открываются в iframe или получают токен через `postMessage`.
...
- Модули проверяют токен через API Avalone.
...
### Фаза 1: Shared session для веток

- Модули «Финансы» и «Работа» принимают токен Avalone.
- Avalone передаёт токен при переходе через URL-параметр или postMessage.
- Модули проверяют токен у Avalone и создают/обновляют локального пользователя.
...
- Становится модулем Avalone: `avalone.online/finance`.

### Ветка «Работа»

- Перестаёт быть самостоятельным приложением с авторизацией.
- Становится модулем Avalone: `avalone.online/work`.

## Open questions

1. Нужно ли мигрировать существующих пользователей старых баз?
...
3. Подготовить модуль «Финансы» к приёму токена Avalone.
4. Протестировать SSO между порталом и модулем «Финансы».
